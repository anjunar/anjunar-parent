...
public class DurationModifiedProvider extends AbstractRestPredicateProvider<Duration, AbstractEntity> {
    @Override
    public Predicate build(Duration value, Identity identity, EntityManager entityManager, CriteriaBuilder builder, Root<AbstractEntity> root, CriteriaQuery<?> query) {
        ...
        return builder.between(root.get(AbstractEntity_.modified), start.toInstant(ZoneOffset.UTC), end.toInstant(ZoneOffset.UTC));
    }
}
